...
@Repository
public class ScreenApprovalDao {
	@Autowired
	DataSource datasource;
	
	@Autowired
	GetRejectedScreen rejectedScreen;
	...
	public List<ScreenApproval> getScreenApproval(String userName, String searchScreenName)
	{
		List<ScreenApproval> lstScreenAproval = new ArrayList<ScreenApproval>();
		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;  
		ScreenApproval screenApproval;
		try
		{
			con = datasource.getConnection();
			if(searchScreenName != null && !searchScreenName.trim().isEmpty())
			{
				pstmt = con.prepareStatement("select screenid, screenname,username,levelnumber,numberoflevel from screen_approver_map where username = ? and screenname like '%"+searchScreenName+"%' order by screenname");
			}else
			{
				pstmt = con.prepareStatement("select screenid, screenname,username,levelnumber,numberoflevel from screen_approver_map where username = ? order by screenname");
			}
			...
			while(rs.next())
			{
				screenApproval = new ScreenApproval();
				screenName = TiimUtil.ValidateNull(rs.getString("screenname")).trim();
				screenApproval.setScreenName(screenName);
				screenApproval.setNoOfLevel(rs.getInt("numberoflevel"));
				int leveNumber = rs.getInt("levelnumber");

				if(screenName.equalsIgnoreCase("Receipt Note"))
				{
					screenApproval.setLstTransactionId(getNonApprovalReceiptNote(leveNumber));
				}else if(screenName.equalsIgnoreCase("Receiving Inspection"))
				{
					screenApproval.setLstTransactionId(getNonApprovalReceivingInspection(leveNumber));
				}else if(screenName.equalsIgnoreCase("Receiving Inspection Request"))
				{
					screenApproval.setLstTransactionId(getNonApprovalReceivingInspectionRequest(leveNumber));
				}else if(screenName.equalsIgnoreCase("Stock Transfer Issue"))
				{
					screenApproval.setLstTransactionId(getNonApprovalStockTransferIssue(leveNumber));
				}else if(screenName.equalsIgnoreCase("Stock Transfer Request"))
				{
					screenApproval.setLstTransactionId(getNonApprovalStockTransferRequest(leveNumber));
				}else if(screenName.equalsIgnoreCase("Production Return Note"))
				{
					screenApproval.setLstTransactionId(getNonApprovalProductionReturnNote(leveNumber));
				}else if(screenName.equalsIgnoreCase("Production Request Note"))
				{
					screenApproval.setLstTransactionId(getNonApprovalToolingRequest(leveNumber));
				}else if(screenName.equalsIgnoreCase("Production Issue Note"))
				{
					screenApproval.setLstTransactionId(getNonApprovalIssueNote(leveNumber));
				}else if(screenName.equalsIgnoreCase("Periodic Inspection Request"))
				{
					screenApproval.setLstTransactionId(getNonApprovalPeriodicRequest(leveNumber));
				}else if(screenName.equalsIgnoreCase("Periodic Inspection Request Report"))
				{
					screenApproval.setLstTransactionId(getNonApprovalPeriodicInspectionReport(leveNumber));
				}else if(screenName.equalsIgnoreCase("Indent Tool"))
				{
					screenApproval.setLstTransactionId(getNonApprovalIndentTool(leveNumber));
				}
				
				screenApproval.setApprovedBy(TiimUtil.ValidateNull(rs.getString("username")).trim());
				screenApproval.setLevelOfApproval(leveNumber);
				screenApproval.setNoOfLevel(rs.getInt("numberoflevel"));
				lstScreenAproval.add(screenApproval);
			}
		...
		return lstScreenAproval;	
	}
	
	public List<ScreenApproval> getRejectedScreenApproval(String userName)
	{
		List<ScreenApproval> lstScreenAproval = new ArrayList<ScreenApproval>();
		Connection con = null;
		PreparedStatement pstmt = null;
		...
		try
		{
			...
			while(rs.next())
			{
				...
				if(screenName.equalsIgnoreCase("Receipt Note"))
				{
					screenApproval.setLstTransactionId(rejectedScreen.getNonApprovalReceiptNote(leveNumber));
				}else if(screenName.equalsIgnoreCase("Receiving Inspection"))
				{
					screenApproval.setLstTransactionId(rejectedScreen.getNonApprovalReceivingInspection(leveNumber));
				}else if(screenName.equalsIgnoreCase("Receiving Inspection Request"))
				{
					screenApproval.setLstTransactionId(rejectedScreen.getNonApprovalReceivingInspectionRequest(leveNumber));
				}else if(screenName.equalsIgnoreCase("Stock Transfer Issue"))
				{
					screenApproval.setLstTransactionId(rejectedScreen.getNonApprovalStockTransferIssue(leveNumber));
				}else if(screenName.equalsIgnoreCase("Stock Transfer Request"))
				{
					screenApproval.setLstTransactionId(rejectedScreen.getNonApprovalStockTransferRequest(leveNumber));
				}else if(screenName.equalsIgnoreCase("Production Return Note"))
				{
					screenApproval.setLstTransactionId(rejectedScreen.getNonApprovalProductionReturnNote(leveNumber));
				}else if(screenName.equalsIgnoreCase("Production Request Note"))
				{
					screenApproval.setLstTransactionId(rejectedScreen.getNonApprovalToolingRequest(leveNumber));
				}else if(screenName.equalsIgnoreCase("Production Issue Note"))
				{
					screenApproval.setLstTransactionId(rejectedScreen.getNonApprovalIssueNote(leveNumber));
				}else if(screenName.equalsIgnoreCase("Periodic Inspection Request"))
				{
					screenApproval.setLstTransactionId(rejectedScreen.getNonApprovalPeriodicRequest(leveNumber));
				}else if(screenName.equalsIgnoreCase("Periodic Inspection Request Report"))
				{
					screenApproval.setLstTransactionId(rejectedScreen.getNonApprovalPeriodicInspectionReport(leveNumber));
				}else if(screenName.equalsIgnoreCase("Indent Tool"))
				{
					screenApproval.setLstTransactionId(rejectedScreen.getNonApprovalIndentTool(leveNumber));
				}
