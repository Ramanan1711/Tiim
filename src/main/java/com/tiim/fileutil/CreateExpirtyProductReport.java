...
public class CreateExpirtyProductReport {
	...
	public void writeProductExcel(List<ToolingReceiptNote> receiptNote, String excelFilePath)
	{
		try{
				...
				for (ToolingReceiptNote product : receiptNote) {
	...
	private void createHeader(Row row)
	{
		...
		cell.setCellValue("Expirty Date");
		cell.setCellStyle(csBold);
	}
	private void writeProductReportBook(ToolingReceiptNote product, Row row) {
		Cell cell = row.createCell(1);
		cell.setCellValue(product.getToolingReceiptId());
		...
		cell = row.createCell(6);
		cell.setCellValue(product.getToolingLotNumber());
