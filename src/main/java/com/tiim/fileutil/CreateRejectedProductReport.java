...
public class CreateRejectedProductReport {
	...
	public void writeProductExcel(List<RejectedProduct> lstReject, String excelFilePath)
	{
		try{
				...
				for (RejectedProduct product : lstReject) {
	...
	private void writeProductReportBook(RejectedProduct product, Row row) {
		Cell cell = row.createCell(1);
		cell.setCellValue(product.getToolingId());

		cell = row.createCell(2);
		cell.setCellValue(product.getProductName());
		
		cell = row.createCell(3);
		cell.setCellValue(product.getMachineName());
		...
		cell = row.createCell(7);
		cell.setCellValue(product.getSerialNumber());
