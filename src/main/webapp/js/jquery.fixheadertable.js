					...
					tab_headers.each(function(i){
						...
						th_div_sort.click(function(){
							...
							if($(this).hasClass('sortedUp')){
								...
								/////////////////Customized For Restrict No of Column to be sort////////////////////
								if ((options.unSortColumn.length != 0))
									{
										for (var j = 0; j < options.unSortColumn.length; j++) {												
											
											if(($(this).text() == options.unSortColumn[j]))
											{
											  return false;
											}											
										}
								...
								if ((options.unSortColumn.length != 0))
								{
									for (var k = 0; k < options.unSortColumn.length; k++) {										
										
										if(($(this).text() == options.unSortColumn[k]))
										{
										  return false;
										}
