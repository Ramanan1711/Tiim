<%@page import="java.util.HashMap, com.tiim.model.MaterialType, com.tiim.model.RoleVsUser "%>
...
$("#techAlert").empty();
    ...
    {	  	
	      ...
	      $("#btndelno").click(function ()      
   	      ...
   	      {		    	   	    	  	    			    		    		    	  
	    	  ...
	    	  $("#frmMaterialType").attr("action","deleteMaterialTypeName.jsf");   	                
	          $("#frmMaterialType").submit(); 
   	      });  	        
   	   ...
   	   $("#searchMaterialType").keypress(function(e){
 	 	  	  if(e.which == 13)
 	 	  	  {	 	  		
 	 	  		 return funAllList();
 	 	  	  }     	  	 
  ...
  function funTypeMaterialTypeName(evt,obj)
  {
	  $("#techAlert").empty();
	   var charCode = (evt.which) ? evt.which : evt.keyCode ;  
	   if(charCode == 13)
	   {
		   var alertmsg = "MaterialType Name is required";
		   var materialTypeName =  trim($("#materialTypeName").val());
		   if(materialTypeName == "")
		   {
			   alertmsg = "MaterialType Name is required";
  ...
  {  	    	  	       	
     ...
     {            	
         $("#action").val("Save");      
         $("#frmMaterialType").attr("action","addMaterialType.jsf");               
     }  
     else if(process == "Update")
     {                           
    	 $("#action").val("Update"); 
    	 $("#frmMaterialType").attr("action","updateMaterialType.jsf");               
     }       
     $("#frmMaterialType").submit(); 	    	        
  }	
   ...
   {		 
	   $.ajax({
			dataType : 'json',
	        url : "${pageContext.request.contextPath}/getMaterialType.jsf?materialTypeId="+id,
	        type : "POST",
	        success : function(result) 
	        {
	    	   ...
	    	   {				
	  	          ...
	  	          $("#materialTypeName").val(trim(result.materialTypeName)); 	
	  	         
	  	          $("#materialTypeId").val(result.materialTypeId);     
	  	          $("#isActive").val(1); 	
   ...
   {	 
       var url = "${pageContext.request.contextPath}/changeMaterialTypeStatus.jsf?materialTypeId="+id;   	  	   
   	 ...
   	 if (request.readyState == 4) 
   	 {
		  if (request.status == 200) 
		  {
			   ...
			   {			   
				    ...
				    {
				    	$("#frmMaterialType").attr("action","pages/SessionExpire.jsp");   	                
					    $("#frmMaterialType").submit();
				    }
				    else
				    {
   ...
   function funAllList()
   {	
	   var searchVal = ($("#searchMaterialType").val());   
	   if(searchVal == "")
  		{   	
		   showAllListDialog();
  		}
 		...
 		{   		    
	 		   ...
	 		   $("#loadoverlay").fadeIn();  
	 		   $("#processloading").fadeIn();	 			                    	                  
	 	 	   $("#frmMaterialType").attr("action","searchMaterialType.jsf");   	                
	 	       $("#frmMaterialType").submit();
  		 }	  
   }
   ...
   {  		    	
	       $("#action").val("listdata");  
 		   $("#loadoverlay").fadeIn();  
 		   $("#processloading").fadeIn();   	 			                    	                  
 	 	   $("#frmMaterialType").attr("action","showMaterialType.jsf");   	                
 	       $("#frmMaterialType").submit();	      	 	
   }   	  
</script>
...
<form name="frmMaterialType" id="frmMaterialType" method="post" autocomplete="off">
  ...
  <div id="overlay" class="web_dialog_overlay"></div>
	    ...
	    <tr>
	       <td>
			  <table style="width:98%" align="center" cellpadding="4" cellspacing="0" border="0">	     
			     <tr>
			        <td align="left" width="50%" class="popuptoptitle">MaterialType Info</td>
			     ...
			     <tr>
			        <td align="left" class="formlabelblack">MaterialType Name&nbsp;<span class="formlabel">*</span></td>
			        <td align="left">&nbsp;<input type="text" class="textsmall" name="materialTypeName" id="materialTypeName" onkeypress="javascript:funTypematerialTypeName(event,this);" value="" autocomplete="off" maxlength="50"/></td>
			     </tr> 
			     ...
			     <tr>
			        <td>&nbsp;</td>
			        <td align="left"><input  type="button" class="btn btnImportant" id="btnprocess" value="Save" onclick="javascript:proccessMaterialType();"/>&nbsp;&nbsp;<input type="button" id="btncancel"  class="btn btnCancel"  onclick="javascript:mstEmpCancelClose();" value="Cancel" /></td>
			     </tr>	
			     <tr>
			       <td colspan="2">&nbsp;</td>				       
		         </tr>	
	...
	<table cellspacing="0" width="100%"  cellpadding="5" border="0" align="center"  style="height:10px">		      
	 ...
	 <table width="1000px" cellspacing="0" cellpadding="5" border="0" align="center"  style="height:25px">		      
	     <tr>  
	          <td width="25%" class="heading" align="left">&nbsp;MaterialType</td> 
		...
		<div style=" width: 1000px; padding:0px; margin: 0px;height:425;min-height:420px;">       
		 ...
		 <div style="width: 980px;background-color:white; border: 1px solid #E0E0E0;;">
		    ...
		    <table cellspacing="0" class="searchList" cellpadding="5" border="0" align="center">
		       <tr>
		          <td class="searchListLabel" align="left">Search&nbsp;&nbsp;<input type="text" name="searchMaterialType" id="searchMaterialType" placeholder="Enter MaterialType Name" value="${searchMaterialType}" class="textmediumlarge" maxlength="50"/>&nbsp;&nbsp;<input type="button" name="btnSearch" id="btnSearch" value="Go" class="btn btnSMImportant"></td>  
		       </tr>
		    </table>  
		    <table cellpadding="0" cellspacing ="0" border="0" align="center"  width="100%" >
			 <tr>
		  ...
		  <c:choose>
			<c:when test="${fn:length(lstMaterialType) gt 0}">
		        <table cellpadding="0" cellspacing ="0" border="0" align="center"  width="100%" >
					 <tr>
						 <td height="5px"></td>		
					 </tr>
			...
			<table id="lstMaterialType" name="lstMaterialType" class="tableHeading">
			      
			     <thead>
			          <tr>
				          <th width="20%">MaterialType Id</th>
				          <th width="30%">MaterialType Name</th>
				          <th width="15%">Action</th>
			          </tr>  
			     </thead> 
			     <tbody>
			         <c:forEach items="${lstMaterialType}" var="lstMaterialType" varStatus="row">  
				          <tr>
				             <td>&nbsp;&nbsp;&nbsp;&nbsp;${lstMaterialType.materialTypeId}  </td>
				             <td>&nbsp;&nbsp;&nbsp;&nbsp;${lstMaterialType.materialTypeName}  </td>
				             
				             <td align="center" style="text-align:center;">&nbsp;&nbsp;&nbsp;&nbsp;
				             	<%
								  if("1".equals(editAccess)){
								%>
				             	<a title="Click to Edit" id="editAnchor${row.index}" href="javascript:funedit('${lstMaterialType.materialTypeId}','${row.index}','${lstMaterialType.isActive}');"><img border="0" id="showedit" src="./images/pencil.png"/></a>&nbsp;
				             	<%
				             	}else{
				             	%>
				             	...
				             	<%}%>    		             					
								<c:choose>
									<c:when test="${lstMaterialType.isActive == 1}">
										<a title="Inactive entry can only be deleted" id="deleteAnchor${row.index}"><img border="0" id="delete${row.index}" src="./images/deleteicon_black.gif"/></a>&nbsp;&nbsp;<a title="Click to make this Inactive" href="javascript:funstatus('${lstMaterialType.materialTypeId}','${row.index}','${lstMaterialType.isActive}');" id="statusAnchor${row.index}" ><img border="0" id="changeStatus${row.index}" style="" src="./images/activate.gif"/></a>
									</c:when>
									<c:when test="${lstMaterialType.isActive == 0}">
										<%
										  if("1".equals(deleteAccess)){
										%>
										<a title="Click to Delete" href="javascript:fundelete('${lstMaterialType.materialTypeId}');" id="deleteAnchor${row.index}" ><img border="0"  id="delete${row.index}" style="" src="./images/deleteicon.gif"/></a>
										<%
										  }else{
						             	%>  
						             	<a title="Access denied" href="#" id="deleteAnchor${row.index}" ><img border="0"  id="delete${row.index}" style="" src="./images/deleteicon.gif"/></a>
						             	<%}%>
										&nbsp;<a title="Click to make this Active" href="javascript:funstatus('${lstMaterialType.materialTypeId}','${row.index}','${lstMaterialType.isActive}');" id="statusAnchor${row.index}" ><img border="0" id="changeStatus${row.index}" style="" src="./images/deactivate.gif"/></a>
									</c:when>
									<c:otherwise>
									    <a title="Cannot delete, MaterialType in use" id="deleteAnchor${row.index}"><img border="0" id="delete${row.index}" src="./images/deleteicon_black.gif"/></a>&nbsp;<a title="Click to make this Active" href="javascript:funstatus('${lstMaterialType.materialTypeId}','${row.index}','${lstMaterialType.isActive}');" id="statusAnchor${row.index}" ><img border="0" style="" id="changeStatus${row.index}" src="./images/deactivate.gif"/></a>
									</c:otherwise>
								</c:choose>
				             </td>
				         </tr>
