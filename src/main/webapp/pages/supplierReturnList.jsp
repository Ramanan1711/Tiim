...
$("#empCode").val("");  
   ...
   $(document).ready(function ()
    {	  	
	      ...
	      {			      	    	  
	    	  ...
	    	  $("#requestNo").val("0"); 
	    	  $("#frnToolRecvInsp").attr("action","showSupplierReturnNote.jsf");   	                
	      ...
	      $("#btndelno").click(function ()      
   	      ...
   	      {		    	   	    	  	    			    		    		    	  
	    	  ...
	    	  $("#frnToolRecvInsp").attr("action","deleteSupplierReturnNote.jsf");   	                
	          $("#frnToolRecvInsp").submit(); 
   	      });  	        
   	   ...
   	   $("#searchReturntNo").keypress(function(e){
 	 	  	  if(e.which == 13)
 	 	  	  {	 	  		
 	 	  		 return funAllList();
 	 	  	  }     	  	 
   ...
   {	
	   $("#returnId").val(id); 
	   $("#frnToolRecvInsp").attr("action","editSupplierReturn.jsf?returnId="+id);   	                
	   $("#frnToolRecvInsp").submit();
   }
   ...
   function funAllList()
   {	
	   var searchVal = ($("#searchReturntNo").val());   
	   if(searchVal == null || searchVal == "" )
 	   ...
 	   {   
		   $("#requestNo").val("0"); 
 		   ...
 		   $("#loadoverlay").fadeIn();  
 		   $("#processloading").fadeIn();	 			                    	                  
 	 	   $("#frnToolRecvInsp").attr("action","searchSupplierReturnNoteList.jsf");   	                
 	       $("#frnToolRecvInsp").submit();
  	   }	  
   }
   ...
   {  		    	
	       ...
	       $("#action").val("listdata");  
 		   $("#loadoverlay").fadeIn();  
 		   $("#processloading").fadeIn();   	 			                    	                  
 	 	   $("#frnToolRecvInsp").attr("action","showSupplierReturnNoteList.jsf");   	                
 	       $("#frnToolRecvInsp").submit();	      	 	
   }   	  
</script>
  ...
  <div id="overlay" class="web_dialog_overlay"></div>
	...
	<table cellspacing="0" width="100%"  cellpadding="5" border="0" align="center"  style="height:10px">		      
	 ...
	 <table width="1000px" cellspacing="0" cellpadding="5" border="0" align="center"  style="height:25px">		      
	     <tr>  
	          <td width="35%" class="heading" align="left">&nbsp;Supplier Return Note</td> 
		...
		<div style=" width: 1000px; padding:0px; margin: 0px;height:425;min-height:420px;">       
		 ...
		 <div style="width: 980px;background-color:white; border: 1px solid #E0E0E0;;">
		    ...
		    <table cellspacing="0" class="searchList" cellpadding="5" border="0" align="center">
		       <tr>
		          <td class="searchListLabel" align="left">Search&nbsp;&nbsp;<input type="text" name="searchReturntNo" id="searchReturntNo" placeholder="Enter Supplier Return No" value="${searchReturntNo}" class="textmediumlarge" autofocus maxlength="50"/>&nbsp;&nbsp;<input type="button" name="btnSearch" id="btnSearch" value="Go" class="btn btnSMImportant"></td>  
		       </tr>
		    </table>  
		    <table cellpadding="0" cellspacing ="0" border="0" align="center"  width="100%" >
			 <tr>
		  ...
		  <c:choose>
			<c:when test="${fn:length(lstSupplierReturn) gt 0}">
		        <table cellpadding="0" cellspacing ="0" border="0" align="center"  width="100%" >
					 <tr>
						 <td height="5px"></td>		
					 </tr>
			...
			<table id="lstFieldTech" name="lstFieldTech" class="tableHeading">
			     ...
			     <thead>
			          <tr>
				          ...
				          <th width="35%">Reason Of Reject</th>
				          <th width="15%">Action</th>
			          </tr>  
			     </thead> 
			     <tbody>
			         <c:forEach items="${lstSupplierReturn}" var="lstSupplierReturn" varStatus="row">  
				          <tr>
				             <td>${lstSupplierReturn.returnId}</td>   
				             <td>${lstSupplierReturn.returnDate}</td>
				             <td>${lstSupplierReturn.requestId}</td>
				             <td align="center" style="text-align:center;">&nbsp;&nbsp;&nbsp;&nbsp;
				             	<%-- <%
								  if("1".equals(editAccess)){
								%>
				             	<a title="Click to Edit" id="editAnchor${row.index}" href="javascript:funedit('${lstSupplierReturn.returnId}');"><img border="0" id="showedit" src="./images/pencil.png"/></a>&nbsp;
				             	<%
								  }else{
				             	%>
								...
								%>
				             	<a title="Click to Delete" href="javascript:fundelete('${lstSupplierReturn.returnId}');" id="deleteAnchor${row.index}" ><img border="0"  id="delete${row.index}" style="" src="./images/deleteicon.gif"/></a>
				             	<%
								  }else{
				             	%>
...
<input type="hidden" name="requestNo" id="requestNo" value="${returnId}"/>
