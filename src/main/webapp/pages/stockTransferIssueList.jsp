...
$("#empCode").val("");  
   ...
   $(document).ready(function ()
    {	  	
	      ...
	      {			      	    	  
	    	  ...
	    	  $("#returnId").val("0"); 
	    	  $("#frmStockTransfer").attr("action","addIntialStockTransferIssue.jsf");   	                
	          $("#frmStockTransfer").submit(); 
	      });
	      ...
	      $("#btndelno").click(function ()      
   	      ...
   	      {		    	   	    	  	    			    		    		    	  
	    	  ...
	    	  $("#frmStockTransfer").attr("action","deleteStockTransferIssue.jsf");   	                
	          $("#frmStockTransfer").submit(); 
   	      });  	        
   	   ...
   	   $("#stockTransferIssueId").keypress(function(e){
   		     $("#returnId").val("0");
 	 	  	  if(e.which == 13)
 	 	  	  {	 	  		
 	 	  		 return funAllList();
   ...
   {		 
	   $("#transferId").val(id); 
	   $("#frmStockTransfer").attr("action","editStockTransferIssue.jsf");   	                
	   $("#frmStockTransfer").submit();
   }
   ...
   function funAllList()
   {	
	   var searchVal = trim($("#searchTransferIssueId").val());   
	   if(searchVal == "")
  	   {   	
		   showAllListDialog();
  	   }
 	   ...
 	   {   
		   $("#transferId").val("0"); 
 		   ...
 		   $("#loadoverlay").fadeIn();  
 		   $("#processloading").fadeIn();	 			                    	                  
 	 	   $("#frmStockTransfer").attr("action","searchStockTransferIssue.jsf");   	                
 	       $("#frmStockTransfer").submit();
  	   }	  
   }
   ...
   {  		    	
	       ...
	       $("#action").val("listdata");  
 		   $("#loadoverlay").fadeIn();  
 		   $("#processloading").fadeIn();   	 			                    	                  
 	 	   $("#frmStockTransfer").attr("action","showStockTransferIssue.jsf");   	                
 	       $("#frmStockTransfer").submit();	      	 	
   }   	  
</script>
...
<form name="frmStockTransfer" id="frmStockTransfer" method="post" autocomplete="off">
		...
		<div style=" width: 1000px; padding:0px; margin: 0px;height:425;min-height:420px;">       
		 ...
		 <div style="width: 980px;background-color:white; border: 1px solid #E0E0E0;;">
		    ...
		    <table cellspacing="0" class="searchList" cellpadding="5" border="0" align="center">
		       <tr>
		          <td class="searchListLabel" align="left">Search&nbsp;&nbsp;<input type="text" name="searchTransferIssueId" id="searchTransferIssueId" placeholder="Enter Stock Issue id" value="${searchTransferIssueId}" class="textmediumlarge" autofocus maxlength="50"/>&nbsp;&nbsp;<input type="button" name="btnSearch" id="btnSearch" value="Go" class="btn btnSMImportant"></td>  
		       </tr>
		    </table>  
		    <table cellpadding="0" cellspacing ="0" border="0" align="center"  width="100%" >
			 <tr>
			...
			<table id="lstFieldTech" name="lstFieldTech" class="tableHeading">
			     ...
			     <thead>
			          <tr>
				          ...
				          <th width="20%">Transfer From Branch</th>
			     ...
			     <tbody>
			         <c:forEach items="${lstStockTransfer}" var="stockTransfer" varStatus="row">  
				          <tr>
				             <td>${stockTransfer.stockTransferIssueId}</td>   
				             <td>${stockTransfer.stockIssueDate}</td>
				             <td>${stockTransfer.fromBranch}</td>
				             <td>${stockTransfer.toBranch}</td>   
				             <td align="center" style="text-align:center;">&nbsp;&nbsp;&nbsp;&nbsp;
				             	<a title="Click to Edit" id="editAnchor${row.index}" href="javascript:funedit('${stockTransfer.stockTransferIssueId}');"><img border="0" id="showedit" src="./images/pencil.png"/></a>&nbsp;
				             	<a title="Click to Delete" href="javascript:fundelete('${stockTransfer.stockTransferIssueId}');" id="deleteAnchor${row.index}" ><img border="0"  id="delete${row.index}" style="" src="./images/deleteicon.gif"/></a>   		             					
				             </td>
				         </tr>
			         </c:forEach>
			     </tbody>       
...
<input type="hidden" name="transferId" id="transferId" value="${transferId}"/>
