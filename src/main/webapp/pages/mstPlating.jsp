...
$("#techAlert").empty();
    ...
    {	  	
	      ...
	      $("#btndelno").click(function ()      
   	      ...
   	      {		    	   	    	  	    			    		    		    	  
	    	  ...
	    	  $("#frmPlating").attr("action","deletePlatingName.jsf");   	                
	          $("#frmPlating").submit(); 
   	      });  	        
   	   ...
   	   $("#searchPlating").keypress(function(e){
 	 	  	  if(e.which == 13)
 	 	  	  {	 	  		
 	 	  		 return funAllList();
 	 	  	  }     	  	 
  ...
  function funTypeRoleName(evt,obj)
  {
	  $("#techAlert").empty();
	   ...
	   if(charCode == 13)
	   {
		   var alertmsg = "Plating Name is required";
		   var platingName =  trim($("#platingName").val());
		   if(platingName == "")
		   {
			   alertmsg = "Plating Name is required";
  ...
  {  	    	  	       	
     ...
     {            	
         $("#action").val("Save");      
         $("#frmPlating").attr("action","addPlating.jsf");               
     }  
     else if(process == "Update")
     {                           
    	 $("#action").val("Update"); 
    	 $("#frmPlating").attr("action","updatePlating.jsf");               
     }       
     $("#frmPlating").submit(); 	    	        
  }	
   ...
   {		 
	   $.ajax({
			dataType : 'json',
	        url : "${pageContext.request.contextPath}/getPlating.jsf?⁯id="+id,
	        type : "POST",
	        success : function(result) 
	        {
	    	   ...
	    	   {				
	  	          ...
	  	          $("#platingName").val(trim(result.platingName)); 	
   ...
   {	 
       var url = "${pageContext.request.contextPath}/changePlatingStatus.jsf?⁯id="+id;   	  	   
   	 ...
   	 if (request.readyState == 4) 
   	 {
		  if (request.status == 200) 
		  {
			   ...
			   {			   
				    ...
				    {
				    	$("#frmPlating").attr("action","pages/SessionExpire.jsp");   	                
					    $("#frmPlating").submit();
				    }
				    else
				    {
   ...
   function funAllList()
   {	
	   var searchVal = ($("#searchPlating").val());   
	   if(searchVal == "")
  		{   	
		   showAllListDialog();
  		}
 		...
 		{   		    
	 		   ...
	 		   $("#loadoverlay").fadeIn();  
	 		   $("#processloading").fadeIn();	 			                    	                  
	 	 	   $("#frmPlating").attr("action","searchPlating.jsf");   	                
	 	       $("#frmPlating").submit();
  		 }	  
   }
   ...
   {  		    	
	       $("#action").val("listdata");  
 		   $("#loadoverlay").fadeIn();  
 		   $("#processloading").fadeIn();   	 			                    	                  
 	 	   $("#frmPlating").attr("action","showPlating.jsf");   	                
 	       $("#frmPlating").submit();	      	 	
   }   	  
</script>
...
<form name="frmPlating" id="frmPlating" method="post" autocomplete="off">
...
<%
	...
	RoleVsUser roleVsUser1 = hmRoleVsUser1.get("Plating");
	if(roleVsUser1 != null)
	{
  ...
  <div id="overlay" class="web_dialog_overlay"></div>
	    ...
	    <tr>
	       <td>
			  <table style="width:98%" align="center" cellpadding="4" cellspacing="0" border="0">	     
			     <tr>
			        <td align="left" width="50%" class="popuptoptitle">Plating Info</td>
			     ...
			     <tr>
			        <td align="left" class="formlabelblack">Plating Name&nbsp;<span class="formlabel">*</span></td>
			        <td align="left">&nbsp;<input type="text" class="textsmall" name="platingName" id="platingName" onkeypress="javascript:funTypePlatingName(event,this);" value="" autocomplete="off" maxlength="50"/></td>
			     </tr> 
			     ...
			     <tr>
			        <td>&nbsp;</td>
			        <td align="left"><input  type="button" class="btn btnImportant" id="btnprocess" value="Save" onclick="javascript:processPlating();"/>&nbsp;&nbsp;<input type="button" id="btncancel"  class="btn btnCancel"  onclick="javascript:mstEmpCancelClose();" value="Cancel" /></td>
			     </tr>	
			     <tr>
			       <td colspan="2">&nbsp;</td>				       
		         </tr>	
	...
	<table cellspacing="0" width="100%"  cellpadding="5" border="0" align="center"  style="height:10px">		      
	 ...
	 <table width="1000px" cellspacing="0" cellpadding="5" border="0" align="center"  style="height:25px">		      
	     <tr>  
	          <td width="25%" class="heading" align="left">&nbsp;Plating</td> 
		...
		<div style=" width: 1000px; padding:0px; margin: 0px;height:425;min-height:420px;">       
		 ...
		 <div style="width: 980px;background-color:white; border: 1px solid #E0E0E0;;">
		    ...
		    <table cellspacing="0" class="searchList" cellpadding="5" border="0" align="center">
		       <tr>
		          <td class="searchListLabel" align="left">Search&nbsp;&nbsp;<input type="text" name="searchPlating" id="searchPlating" placeholder="Enter Plating Name" value="${searchPlating}" class="textmediumlarge" maxlength="50"/>&nbsp;&nbsp;<input type="button" name="btnSearch" id="btnSearch" value="Go" class="btn btnSMImportant"></td>  
		       </tr>
		    </table>  
		    <table cellpadding="0" cellspacing ="0" border="0" align="center"  width="100%" >
			 <tr>
		  ...
		  <c:choose>
			<c:when test="${fn:length(latPlating) gt 0}">
		        <table cellpadding="0" cellspacing ="0" border="0" align="center"  width="100%" >
					 <tr>
						 <td height="5px"></td>		
					 </tr>
			...
			<table id="latPlating" name="latPlating" class="tableHeading">
			      
			     <thead>
			          <tr>
				          <th width="20%">Plating Id</th>
				          <th width="30%">Plating Name</th>
				          <th width="15%">Action</th>
			          </tr>  
			     </thead> 
			     <tbody>
			         <c:forEach items="${latPlating}" var="latPlating" varStatus="row">  
				          <tr>
				             <td>&nbsp;&nbsp;&nbsp;&nbsp;${latPlating.⁯id}  </td>
				             <td>&nbsp;&nbsp;&nbsp;&nbsp;${latPlating.platingName}  </td>
				             
				             <td align="center" style="text-align:center;">&nbsp;&nbsp;&nbsp;&nbsp;
				             	<%
								  if("1".equals(editAccess)){
								%>
				             	<a title="Click to Edit" id="editAnchor${row.index}" href="javascript:funedit('${latPlating.⁯id}','${row.index}','${latPlating.active}');"><img border="0" id="showedit" src="./images/pencil.png"/></a>&nbsp;
				             	<%
				             	}else{
				             	%>
				             	...
				             	<%}%>    		             					
								<c:choose>
									<c:when test="${latPlating.active == 1}">
										<a title="Inactive entry can only be deleted" id="deleteAnchor${row.index}"><img border="0" id="delete${row.index}" src="./images/deleteicon_black.gif"/></a>&nbsp;&nbsp;<a title="Click to make this Inactive" href="javascript:funstatus('${latPlating.⁯id}','${row.index}','${latPlating.delStatus}');" id="statusAnchor${row.index}" ><img border="0" id="changeStatus${row.index}" style="" src="./images/activate.gif"/></a>
									</c:when>
									<c:when test="${latPlating.active == 0}">
										<%
										  if("1".equals(deleteAccess)){
										%>
										<a title="Click to Delete" href="javascript:fundelete('${latPlating.⁯id}');" id="deleteAnchor${row.index}" ><img border="0"  id="delete${row.index}" style="" src="./images/deleteicon.gif"/></a>
										<%
										  }else{
						             	%>  
						             	<a title="Access denied" href="#" id="deleteAnchor${row.index}" ><img border="0"  id="delete${row.index}" style="" src="./images/deleteicon.gif"/></a>
						             	<%}%>
										&nbsp;<a title="Click to make this Active" href="javascript:funstatus('${latPlating.⁯id}','${row.index}','${latPlating.active}');" id="statusAnchor${row.index}" ><img border="0" id="changeStatus${row.index}" style="" src="./images/deactivate.gif"/></a>
									</c:when>
									<c:otherwise>
									    <a title="Cannot delete, Plating in use" id="deleteAnchor${row.index}"><img border="0" id="delete${row.index}" src="./images/deleteicon_black.gif"/></a>&nbsp;<a title="Click to make this Active" href="javascript:funstatus('${latPlating.⁯id}','${row.index}','${latPlating.active}');" id="statusAnchor${row.index}" ><img border="0" style="" id="changeStatus${row.index}" src="./images/deactivate.gif"/></a>
									</c:otherwise>
								</c:choose>
				             </td>
				         </tr>
