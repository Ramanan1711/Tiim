  ...
  <div id="overlay" class="web_dialog_overlay"></div>
	...
	<center>           
		<div style=" width: 1000px; padding:0px; margin: 0px;height:425;min-height:420px;">       
		  ...
		  <c:choose>
			<c:when test="${fn:length(toolDestructionNotes) gt 0}">
			...
			<table id="lstProduct" name="lstProduct" class="tableHeading">			     
			     <thead>
			          <tr>
				          ...
				          <th width="15%">Destruction By</th>
				          <th width="15%">Rejected Qty</th>
				          <th width="30%">Date</th>
			          </tr>  
			     </thead> 
			     <tbody>
			         <c:forEach items="${toolDestructionNotes}" var="toolDestructionNote" varStatus="row">  
				          <tr>
				           <td>	
				               ${toolDestructionNote.lotNumber}</td>   
				             <td>	
				               ${toolDestructionNote.module}</td>   
				             <td>${toolDestructionNote.serialNumber}</td>
				             <td>${toolDestructionNote.destructionBy}</td>
				             <td>${toolDestructionNote.rejectedQty}</td>
				             <td>${toolDestructionNote.creationDate}</td>
