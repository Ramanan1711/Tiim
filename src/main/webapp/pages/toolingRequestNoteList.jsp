...
$("#empCode").val("");  
   ...
   $(document).ready(function ()
    {	  	
	      ...
	      {			      	    	  
	    	  ...
	    	  $("#requestId").val("0"); 
	    	  $("#frmToolReqNote").attr("action","addIntialToolingRequestNote.jsf");   	                
	      ...
	      $("#btndelno").click(function ()      
   	      ...
   	      {		    	   	    	  	    			    		    		    	  
	    	  ...
	    	  $("#frmToolReqNote").attr("action","deleteRequestNote.jsf");   	                
	          $("#frmToolReqNote").submit(); 
   	      });  	        
   ...
   {		 
	   $("#requestId").val(id); 
	   $("#frmToolReqNote").attr("action","editToolingRequest.jsf");   	                
	   $("#frmToolReqNote").submit();
   }
   ...
   {	 
	   ...
	   window.open('viewToolingRequestReport.jsf?requestId='+requestId, 'window', 'width=700,height=750'); 
   }
	   ...
	   if(searchVal == "")
 	   ...
 	   {   
		   $("#requestId").val("0"); 
 		   ...
 		   $("#loadoverlay").fadeIn();  
 		   $("#processloading").fadeIn();	 			                    	                  
 	 	   $("#frmToolReqNote").attr("action","searchToolingRequest.jsf");   	                
 	       $("#frmToolReqNote").submit();
  	   }	  
   }
   ...
   {  		    	
	       ...
	       $("#action").val("listdata");  
 		   $("#loadoverlay").fadeIn();  
 		   $("#processloading").fadeIn();   	 			                    	                  
 	 	   $("#frmToolReqNote").attr("action","showToolingRequest.jsf");   	                
 	       $("#frmToolReqNote").submit();	      	 	
   }   	  
</script>
...
<form name="frmToolReqNote" id="frmToolReqNote" method="post" autocomplete="off">
 <%@ include file="tiimMenu.jsp"%>
  ...
  <%
	 ...
	 RoleVsUser roleVsUser1 = hmRoleVsUser1.get("Production Request Note");
	 if(roleVsUser1 != null)
	 {
  ...
  <div id="overlay" class="web_dialog_overlay"></div>
	...
	<table cellspacing="0" width="100%"  cellpadding="5" border="0" align="center"  style="height:10px">		      
	 ...
	 <table width="1000px" cellspacing="0" cellpadding="5" border="0" align="center"  style="height:25px">		      
	     <tr>  
	          <td width="30%" class="heading" align="left">&nbsp;Production Tool Request</td> 
			...
			<table id="lstFieldTech" name="lstFieldTech" class="tableHeading">
			     ...
			     <tbody>
			         <c:forEach items="${lstToolingRequestNote}" var="lstToolingRequestNote" varStatus="row">  
				          <tr>
				             <td>${lstToolingRequestNote.originalId}</td>   
				             <td>${lstToolingRequestNote.requestDate}</td>
				             <td>${lstToolingRequestNote.requestBy}</td>
				             ...
				             <%
								...
								%>
								 <c:choose>
									<c:when test="${lstToolingRequestNote.reportStatus eq 0}">
									      	<a title="Click to Edit" id="editAnchor${row.index}" href="javascript:funedit('${lstToolingRequestNote.requestId}');"><img border="0" id="showedit" src="./images/pencil.png"/></a>&nbsp;
									 </c:when>
									 <c:otherwise>
								...
								%>
				             	<a title="Click to Delete" href="javascript:fundelete('${lstToolingRequestNote.originalId}','${lstToolingRequestNote.requestId}');" id="deleteAnchor${row.index}" ><img border="0"  id="delete${row.index}" style="" src="./images/deleteicon.gif"/></a>
				             	<%
								 }else{
				             	%>
								...
								%>
				             	<a title="Click to View Report" href="javascript:viewReport('${lstToolingRequestNote.requestId}');" ><img border="0"  id="view${row.index}" style="" src="./images/view_report-icon.png"/></a>
				             	<%
								 }else{
				             	%>
