...
$("#empCode").val("");  
   ...
   $(document).ready(function ()
    {	  	
		   $('.tableHeading').fixheadertable({
	       	   caption     : '', 
	           colratio    : [100, 100, 150, 180, 180, 160, 100], 
	           height      : 350, 
	           width       : 970,             
	           sortable    : true, 
	           sortedColId : 0,    
	           unSortColumn: ['Report No'],     
	           resizeCol   : true,            
	           minColWidth : 50,
	           whiteSpace	 : 'nowrap',
	           pager		 : true,
	      ...
	      {			      	    	  
	    	  ...
	    	  $("#reportNo").val("0"); 
	    	  $("#frmToolReturn").attr("action","addIntialPeriodicInspectionReport.jsf");   	                
	      ...
	      $("#btndelno").click(function ()      
   	      ...
   	      {		    	   	    	  	    			    		    		    	  
	    	  ...
	    	  $("#frmToolReturn").attr("action","deletePeriodicInspectionReport.jsf");   	                
	          $("#frmToolReturn").submit(); 
   	      });  	        
   	   ...
   	   $("#searchReportNo").keypress(function(e){
   		     $("#reportNo").val("0");
 	 	  	  if(e.which == 13)
 	 	  	  {	 	  		
 	 	  		 return funAllList();
   ...
   {		 
	   $("#reportNo").val(id); 
	   $("#frmToolReturn").attr("action","editPeriodicInspectionReport.jsf");   	                
	   $("#frmToolReturn").submit();
   }
   ...
   {		 
	   $("#reportNo").val(id); 
	   window.open('viewPeriodicInspectionReport.jsf?reportNo='+id, 'window', 'width=700,height=750'); 
   }
   ...
   function funAllList()
   {	
	   var searchVal = trim($("#searchReportNo").val());   
	   if(searchVal == "")
  	   {   	
		   showAllListDialog();
  	   }
 	   ...
 	   {   
		   $("#reportNo").val("0"); 
 		   ...
 		   $("#loadoverlay").fadeIn();  
 		   $("#processloading").fadeIn();	 			                    	                  
 	 	   $("#frmToolReturn").attr("action","showToolingPeriodicInspectionReport.jsf");   	                
 	       $("#frmToolReturn").submit();
  	   }	  
   }
   ...
   {  		    	
	       ...
	       $("#action").val("listdata");  
 		   $("#loadoverlay").fadeIn();  
 		   $("#processloading").fadeIn();   	 			                    	                  
 	 	   $("#frmToolReturn").attr("action","searchToolingPeriodicInspectionReport.jsf");   	                
 	       $("#frmToolReturn").submit();	      	 	
   }   	  
</script>
...
<form name="frmToolReturn" id="frmToolReturn" method="post" autocomplete="off">
 ...
 <%
	...
	RoleVsUser roleVsUser1 = hmRoleVsUser1.get("Periodic Inspection Request Report");
	if(roleVsUser1 != null)
	{
		...
		<div style=" width: 1000px; padding:0px; margin: 0px;height:425;min-height:420px;">       
		 ...
		 <div style="width: 980px;background-color:white; border: 1px solid #E0E0E0;;">
		    ...
		    <table cellspacing="0" class="searchList" cellpadding="5" border="0" align="center">
		       <tr>
		          <td class="searchListLabel" align="left">Search&nbsp;&nbsp;<input type="text" name="searchReportNo" id="searchReportNo" placeholder="Enter Report No" value="${searchReportNo}" class="textmediumlarge" autofocus maxlength="50"/>&nbsp;&nbsp;<input type="button" name="btnSearch" id="btnSearch" value="Go" class="btn btnSMImportant"></td>  
		       </tr>
		    </table>  
		    <table cellpadding="0" cellspacing ="0" border="0" align="center"  width="100%" >
			 <tr>
			...
			<table id="lstFieldTech" name="lstFieldTech" class="tableHeading">
			     ...
			     <thead>
			          <tr>
				          ...
				          <th width="15%">Report By</th>
				          <th width="10%">Action</th>
			          </tr>  
			     </thead> 
			     <tbody>
			         <c:forEach items="${lstPeriodicInspectionReport}" var="arlInspectionReport" varStatus="row">  
				          <tr>
				             <td>${arlInspectionReport.originalId}</td>   
				             <td>${arlInspectionReport.reportDate}</td>
				             <td>${arlInspectionReport.requestNo}</td>
				             <td>${arlInspectionReport.requestDate}</td>   
				             <td>${arlInspectionReport.requestedBy}</td>
				             <td>${arlInspectionReport.reportedBy}</td>
				             <td align="center" style="text-align:center;">&nbsp;&nbsp;&nbsp;&nbsp;
			             		<%
								  if("1".equals(editAccess)){
								%>
				             	<a title="Click to Edit" id="editAnchor${row.index}" href="javascript:funedit('${arlInspectionReport.reportNo}');"><img border="0" id="showedit" src="./images/pencil.png"/></a>&nbsp;
				             	<%
								  }else{
				             	%>
				             	<a title="Click to Edit" id="editAnchor${row.index}" href="#"><img border="0" id="showedit" src="./images/pencil.png"/></a>&nbsp;
				             	<%}%>
				             	<%
								  if("1".equals(deleteAccess)){
								%>
				             	<a title="Click to Delete" href="javascript:fundelete('${arlInspectionReport.originalId}','${arlInspectionReport.reportNo}');" id="deleteAnchor${row.index}" ><img border="0"  id="delete${row.index}" style="" src="./images/deleteicon.gif"/></a>
				             	<%
								  }else{
				             	%>
								...
								%>
				             	<a title="Click to Delete" href="javascript:viewReport('${arlInspectionReport.reportNo}');" ><img border="0"  id="view${row.index}" style="" src="./images/view_report-icon.png"/></a>
				             	<%
								  }else{
				             	%>
