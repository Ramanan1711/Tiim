...
$("#empCode").val("");  
   ...
   $(document).ready(function ()
    {	  	
	      ...
	      {			      	    	  
	    	  ...
	    	  $("#returnId").val("0"); 
	    	  $("#frmStockTransfer").attr("action","addIntialStockTransfer.jsf");   	                
	      ...
	      $("#btndelno").click(function ()      
   	      ...
   	      {		    	   	    	  	    			    		    		    	  
	    	  ...
	    	  $("#frmStockTransfer").attr("action","deleteStockTransfer.jsf");   	                
	          $("#frmStockTransfer").submit(); 
   	      });  	        
   	   ...
   	   $("#searchTransferId").keypress(function(e){
   		     $("#returnId").val("0");
 	 	  	  if(e.which == 13)
 	 	  	  {	 	  		
 	 	  		 return funAllList();
   ...
   {		 
	   $("#transferId").val(id); 
	   $("#frmStockTransfer").attr("action","editStockTransfer.jsf");   	                
	   $("#frmStockTransfer").submit();
   }
   ...
   function funAllList()
   {	
	   var searchVal = trim($("#searchTransferId").val());   
	   if(searchVal == "")
  	   {   	
		   showAllListDialog();
  	   }
 	   ...
 	   {   
		   $("#transferId").val("0"); 
 		   ...
 		   $("#loadoverlay").fadeIn();  
 		   $("#processloading").fadeIn();	 			                    	                  
 	 	   $("#frmStockTransfer").attr("action","searchStockTransfer.jsf");   	                
 	       $("#frmStockTransfer").submit();
  	   }	  
   }
   ...
   {  		    	
	       ...
	       $("#action").val("listdata");  
 		   $("#loadoverlay").fadeIn();  
 		   $("#processloading").fadeIn();   	 			                    	                  
 	 	   $("#frmStockTransfer").attr("action","showStockTransfer.jsf");   	                
 	       $("#frmStockTransfer").submit();	      	 	
   }   	  
</script>
		...
		<div style=" width: 1000px; padding:0px; margin: 0px;height:425;min-height:420px;">       
		 ...
		 <div style="width: 980px;background-color:white; border: 1px solid #E0E0E0;;">
		    ...
		    <table cellspacing="0" class="searchList" cellpadding="5" border="0" align="center">
		       <tr>
		          <td class="searchListLabel" align="left">Search&nbsp;&nbsp;<input type="text" name="searchTransferId" id="searchTransferId" placeholder="Enter Transfer id" value="${searchTransferId}" class="textmediumlarge" autofocus maxlength="50"/>&nbsp;&nbsp;<input type="button" name="btnSearch" id="btnSearch" value="Go" class="btn btnSMImportant"></td>  
		       </tr>
		    </table>  
		    <table cellpadding="0" cellspacing ="0" border="0" align="center"  width="100%" >
			 <tr>
			...
			<table id="lstFieldTech" name="lstFieldTech" class="tableHeading">
			     ...
			     <tbody>
			         <c:forEach items="${lstStockTransfer}" var="stockTransfer" varStatus="row">  
				          <tr>
				             <td>${stockTransfer.stockTransferId}</td>   
				             <td>${stockTransfer.stockDate}</td>
				             <td>${stockTransfer.fromBranch}</td>
				             <td>${stockTransfer.toBranch}</td>   
				             <td align="center" style="text-align:center;">&nbsp;&nbsp;&nbsp;&nbsp;
				             	<a title="Click to Edit" id="editAnchor${row.index}" href="javascript:funedit('${stockTransfer.stockTransferId}');"><img border="0" id="showedit" src="./images/pencil.png"/></a>&nbsp;
				             	<a title="Click to Delete" href="javascript:fundelete('${stockTransfer.stockTransferId}');" id="deleteAnchor${row.index}" ><img border="0"  id="delete${row.index}" style="" src="./images/deleteicon.gif"/></a>   		             					
				             </td>
				         </tr>
			         </c:forEach>
			     </tbody>       
